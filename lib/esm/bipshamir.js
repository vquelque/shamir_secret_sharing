...
export class BIPShamir {
    ...
    constructor(numShares, minRecovery) {
        if (numShares < config.MIN_NUMBER_OF_SHARES ||
    ...
    splitHex(secret) {
        ...
        const pointsByShare = hexStringToUint8Array(secret).map((s) => {
            ...
            const polynomial = PolynomialGF256.getRandomPoly(this.minRecovery - 1, new Uint8Array([s]));
    ...
    createEncodedShares(secret) {
        ...
        return secret.map((share) => {
            const v_string = `V${leftPad(version.toString(16), "0", 2)}M${leftPad(this.minRecovery.toString(16), "0", 2)}P${leftPad(share.x, "0", 2)}`;
